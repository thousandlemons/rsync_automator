...
```json
{
  ...
  "ssh_port": 22,
  "ssh_key_file": "/home/my username/.ssh/id_ed25519",
...
| `ssh_key_file`                   | Optional  | String     | The path of the ssh key file on the local machine. Defining this field will add `-e "ssh -i \"{ssh_key_file}\""` argument to rsync. This option can be combined with the `ssh_port` option above. If both options are set, there will only be one combined `-e "ssh {ssh_args}"` added.                                               |
